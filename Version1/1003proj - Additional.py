...
studentsPerGrp = int(input('Enter no of students per grp from 4 to 10: '))
...
with open(file_path, mode='r', newline='') as file:
    ...
    for eachClass in sorted_classes:
        ...
        for student in eachClass:
            if student[4] == 'Male':
                ...
                FemaleCounter+=1
        GenderList.append([round(MaleCounter/noOfGrps,3),round(FemaleCounter/noOfGrps,3)])
    ...
    for eachClass in sorted_classes:
        ...
        while len(GrpList) < noOfGrps:
            ...
            while len(Grp) < studentsPerGrp:  # We need 5 members in each group
            ...
            #shd have the min no of members per grp now           
            GrpList.append(Grp)

        #check if got leftover student
        if len(eachClass)>0:
            #sort according to GPA, lowest GPA first ele
            sorted_leftovers = sorted(eachClass, key=lambda x: float(x[5]), reverse=False)
            #print(sorted_leftovers)
            #sort Grp according to GPA, first ele is Grp with lowest avg GPA
            sorted_GrpList = sorted(GrpList, key=lambda group: sum(float(student[5]) for student in group) / len(group), reverse=False)
            #print(sorted_GrpList)
